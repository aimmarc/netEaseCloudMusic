...
class PlayTopPage extends StatefulWidget {
  @override
  PlayTopState createState() => PlayTopState();
}

class PlayTopState extends State with AutomaticKeepAliveClientMixin {
  ...
  Future _getPlayList() async {
    ...
    try {
      final response = await DioUtil.getInstance().post(
        "$API_PREFIX/top/playlist/highquality?limit=30&before=$_before",
        {},
      );
      setState(() {
        _isLoading = false;
